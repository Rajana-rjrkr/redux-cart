...
const Header = ({insideHeader}) => {
  ...
  return (
    <nav className='flex w-full bg-rose-800 text-white font-bold fixed p-5 justify-between text-lg'>
      ...
      <ul className='flex'>
        {
        insideHeader && <li className='mx-5'><input onChange={e=>dispatch(searchProducts(e.target.value))} type="text" placeholder='Search Products Here...' className='border p-1 rounded' /></li>
        }
